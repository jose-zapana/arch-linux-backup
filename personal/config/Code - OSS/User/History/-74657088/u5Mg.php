...
Route::get('prueba', function () {
    $data = Quotation::first(); // Cambiar el nombre de la variable aquí
    $pdf = Pdf::loadView('admin.pdf.quotation', compact('data'))->setPaper('A4');

    $pdf->save(storage_path('app/public/quotations/' . $data->id . '.pdf'));

    return $pdf->stream();

    return view('admin.pdf.quotation', compact('data')); // Asegúrate de usar 'data'
});
