...
class ShippingAddresses extends Component
{
    ...
    public function mount()
    {
        ...
        {
            // Asegúrate de que las direcciones sean una colección de Eloquent
            $this->addresses = Address::where('user_id', auth()->id())->get() ?? collect();
        
            // Emite las direcciones al componente hijo como colección
            $this->emitTo('summary', 'updateAddresses', $this->addresses->toArray()); // Usar toArray() para convertir la colección a un array
        }
    ...
    public function update()
    {
        ...
        $this->emit('updateAddresses', Address::where('user_id', auth()->id())->get());
