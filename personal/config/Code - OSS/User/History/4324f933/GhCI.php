    ...
    <div class="max-w-3xl mx-auto pt-12">
        ...
        @if (session('niubiz'))
            ...
            @php
                ...
                $response = $izipay['response'] ?? [];
                $transactionStatus = $response['transactions'][0]['status'] ?? 'Estado no disponible';
                $orderId = $response['orderDetails']['orderId'] ?? 'N/A';
                $orderTotalAmount = isset($response['orderDetails']['orderTotalAmount']) ? number_format($response['orderDetails']['orderTotalAmount'] / 100, 2) : 'Monto no disponible';
                $orderCurrency = $response['orderDetails']['orderCurrency'] ?? 'USD';
                
                // Obtener la fecha de la transacción desde el campo 'serverDate'
                $serverDate = isset($response['orderDetails']['serverDate']) ? \Carbon\Carbon::parse($response['orderDetails']['serverDate'])->format('d/m/Y H:i:s') : 'Fecha no disponible';
            @endphp
            ...
            <div class="flex items-center p-4 mb-4 text-sm text-blue-800 border border-blue-300 rounded-lg bg-blue-50 dark:bg-gray-800 dark:text-blue-400 dark:border-blue-800"
                ...
                <div>
                    ...
                    <p><strong>Número de Pedido:</strong> {{ $izipay['purchasenumber'] ?? 'N/A' }}</p>
                    <p><strong>Monto:</strong> 
                        {{ isset($response['orderDetails']['orderTotalAmount']) ? number_format($response['orderDetails']['orderTotalAmount'] / 100, 2) : 'Monto no disponible' }} 
                        {{ $response['orderDetails']['orderCurrency'] ?? '' }}
                    </p>
                </div>
            </div>
