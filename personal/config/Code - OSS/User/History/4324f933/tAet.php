<x-app-layout>
    ...
    <div class="max-w-3xl mx-auto pt-12">
        ...
        @if (session('niubiz'))
            ...
            $transactionStatus = $response['orderStatus'] ?? 'Estado no disponible';
            $orderId = $response['orderDetails']['orderId'] ?? 'N/A';
            $orderTotalAmount = isset($response['orderDetails']['orderTotalAmount']) 
                ? number_format($response['orderDetails']['orderTotalAmount'] / 100, 2) 
                : 'Monto no disponible';
            $orderCurrency = $response['orderDetails']['orderCurrency'] ?? 'PEN';
            $serverDate = isset($response['serverDate']) 
                ? \Carbon\Carbon::parse($response['serverDate'])->format('d/m/Y H:i:s') 
                : 'Fecha no disponible';
            @endphp
