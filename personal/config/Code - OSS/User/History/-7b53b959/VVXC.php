<x-app-layout>
    ...
    <!-- Slider principal -->
    <div class="swiper mb-12 hidden md:block">
    ...
    <!-- Últimos productos -->
    <x-container>
        <h1 class="text-2xl font-bold text-gray-700 dark:text-gray-300 mb-4 mt-10 text-center uppercase">
            Últimos productos
        </h1>        
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            @foreach ($lastProducts as $product)
                <article class="bg-white dark:bg-gray-800 overflow-hidden sm:rounded-lg flex flex-col h-full">
                    ...
                    <figure class="product-img"> <a href="{{ route('products.show', $product) }}" aria-label="Ver detalles del producto: {{ $product->name }}"> <img fetchpriority="high" src="{{ $product->getFirstMediaUrl('images', 'product') }}" alt="Imagen del producto: {{ $product->name }}" class="w-full object-cover object-center" width="400" height="400"> 
                    </figure>
        ...
        <h1 class="text-2xl font-bold text-gray-700 dark:text-gray-300 mb-4 text-center uppercase mt-10 flex items-center justify-center">
            Últimas Entradas
            <a href="{{ route('posts.index') }}" class="ml-3 inline-block bg-indigo-500 text-white text-sm font-semibold py-2 px-4 rounded-lg hover:bg-indigo-600 transition-all duration-300">Ver todo</a>
        </h1>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach ($posts as $post)
                <article class="w-full h-80 bg-cover bg-center @if($loop->first) md:col-span-2 @endif rounded-lg"
                         style="background-image: url('{{ $post->getFirstMediaUrl('images', 'posts') ?: 'https://cdn.pixabay.com/photo/2018/01/14/23/12/nature-3082832_960_720.jpg' }}')">
