<x-app-layout>
    @push('css')
        ...
        @isset($lastProducts)
            @foreach ($lastProducts as $product)
                <link rel="preload" href="{{ $product->getFirstMediaUrl('images', 'webp_small') }}" as="image" type="image/webp">
            @endforeach
        @endisset
    ...
    <x-container>
                ...
                <article class="bg-white dark:bg-gray-800 overflow-hidden sm:rounded-lg flex flex-col h-full">
                    <figure class="product-img">
                        <a href="{{ route('products.show', $product) }}" aria-label="Ver detalles del producto: {{ $product->name }}">
                            <img fetchpriority="high" src="{{ $product->getFirstMediaUrl('images', 'mobile_small') }}" 
        ...
        <article class="w-full h-80 bg-cover bg-center @if($loop->first) md:col-span-2 @endif rounded-lg" style="background-image: url(@if($post->image){{ Storage::url($post->image->url) }}@else https://cdn.pixabay.com/photo/2018/01/14/23/12/nature-3082832_960_720.jpg @endif)">
            <div class="w-full h-full px-8 flex flex-col justify-center">
                ...
                <h1 class="text-2xl font-bold leading-8 mt-2 text-white">
