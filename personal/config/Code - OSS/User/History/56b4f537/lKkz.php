<div x-data="{ open: false }">
                            ...
                            @guest
                                <div class="px-4 py-2 bg-gray-800 dark:bg-gray-900">
                                    ...
                                    <p class="text-sm text-center mt text-gray-300 dark:text-gray-400">
                                        ¿No tienes cuenta?
                                        <a href="{{ route('register') }}" class="text-[#00a1b4] hover:underline mb-2 font-semibold">
                                            Regístrate
                                        </a>
                                    </p>
                                </div>
    ...
    <div x-show="open" x-on:click="open=false" style="display: none"
        class="fixed top-0 left-0 inset-0 bg-black opacity-25 z-10 md:hidden">
    </div>

    <div x-show="open" style="display: none" class="fixed top-0 left-0 z-20 w-64 md:w-80 h-full bg-white dark:bg-gray-800">
            ...
            <ul>
                @foreach ($families as $family)
                    <li wire:mouseover="set('family_id', {{ $family->id }})">
                        <a href="{{ route('families.show', $family) }}" class="flex px-4 py-4 items-center justify-between hover:bg-gray-200 dark:hover:bg-gray-700">
                            {{ $family->name }}
        ...
        <div class="bg-white dark:bg-gray-800 h-[calc(100vh-52px)] overflow-auto px-6 py-8">
            ...
            <ul class="grid grid-cols-1 xl:grid-cols-3">
                @foreach ($this->categories as $category)
                    <li>
                        <a href="{{ route('categories.show', $category) }}" class="text-[#00a1b4] font-semibold text-lg">
                            {{ $category->name }}
                        </a>
                        <ul class="mt-4 space-y-2">
                            @foreach ($category->subcategories as $subcategory)
                                <li>
                                    <a href="{{ route('subcategories.show', $subcategory) }}" class="text-gray-600 dark:text-gray-400 text-sm hover:text-[#00a1b4]">
                                        {{ $subcategory->name }}
                                    </a>
                                </li>
                            @endforeach
