...
class CheckoutController extends Controller
{
    ...
    {
        try {
            // Validar el algoritmo hash
            if ($request->get('kr-hash-algorithm') !== 'sha256_hmac') {
                return redirect()->route('checkout.index')->with('error', 'Algoritmo de hash inválido.');
            }
    
            // Decodificar la respuesta de IZIPAY
            $krAnswer = str_replace('\/', '/', $request->get('kr-answer'));
    
            // Calcular el hash y validarlo
            $calculatedHash = hash_hmac('sha256', $krAnswer, config('services.izipay.hash_key'));
            if ($calculatedHash !== $request->get('kr-hash')) {
                return redirect()->route('checkout.index')->with('error', 'El hash es inválido.');
            }
    
            // Decodificar la respuesta en formato JSON
            $response = json_decode($krAnswer, true);
    
            // Verificar si la respuesta contiene los datos necesarios
            if (!isset($response['orderDetails']['orderStatus']) || $response['orderDetails']['orderStatus'] !== 'PAID') {
                return redirect()->route('checkout.index')->with('error', 'La transacción no fue exitosa o no fue pagada.');
            }
    
            // Obtener información de la transacción
            $transactionId = $response['answer']['transactionId'] ?? null;
            $amount = $response['answer']['amount'] ?? 0;
    
            // Validar el contenido del carrito
            $content = Cart::instance('shopping')->content()->filter(function ($item) {
                return $item->qty <= $item->options['stock'];
            });
    
            if ($content->isEmpty()) {
                return redirect()->route('checkout.index')->with('error', 'El carrito está vacío o los productos no tienen stock suficiente.');
            }
            ...
            if (!$address) {
                return redirect()->route('checkout.index')->with('error', 'No se encontró una dirección predeterminada.');
            }
            ...
            return redirect()->route('checkout.index')->with('error', 'Hubo un error al procesar la transacción: ' . $e->getMessage());
        }
    }
