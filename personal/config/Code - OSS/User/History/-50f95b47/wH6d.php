...
class CheckoutController extends Controller
{
    ...
    public function izipay(Request $request)
    {
        ...
        // Decodificar el contenido de kr-answer
        $krAnswerDecoded = json_decode($krAnswer, true);

        // Obtener datos de la transacción
        $serverDate = isset($krAnswerDecoded['orderDetails']['serverDate']) 
            ? Carbon::parse($krAnswerDecoded['orderDetails']['serverDate'])->format('d/m/Y H:i:s') 
            : 'Fecha no disponible';

        // Redirigir con los datos necesarios
        return redirect()->route('gracias')->with('izipay', [
            'response' => $krAnswerDecoded,
            'purchasenumber' => $krAnswerDecoded['orderDetails']['orderId'] ?? 'N/A',
            'serverDate' => $serverDate,
            'transactionStatus' => $krAnswerDecoded['transactions'][0]['status'] ?? 'Estado no disponible',
            'orderTotalAmount' => number_format(($krAnswerDecoded['orderDetails']['orderTotalAmount'] ?? 0) / 100, 2),
        ]);
