...
class CheckoutController extends Controller
{
    ...
    public function izipay(Request $request)
    {
        ...
        // Almacenar los datos en una sesión flash
        session()->flash('izipay', [
            'response' => $krAnswerDecoded,   
        ]);


        if (isset($response['orderStatus']) && $response['orderStatus'] == 'PAID') {
            $orderId = $response['orderDetails']['orderId'] ?? null;
            $totalAmount = $response['orderDetails']['orderTotalAmount'] ?? 0;
            
            if ($orderId && $totalAmount > 0) {
                // Lógica para procesar el pedido o actualizar el estado del pedido
                // Por ejemplo, guardar el pedido en la base de datos
                // Redirigir o hacer algo según lo necesites

                // Redirigir a la página 'gracias'
                return redirect()->route('gracias');
            } else {
                // Si el ID de la orden o el monto es incorrecto, manejar el error
            }
        } else {
            // Si el pago no fue exitoso, manejar el error o la falta de pago
        }
