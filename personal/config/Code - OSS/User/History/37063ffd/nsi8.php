<x-admin-layout :breadcrumbs="[
    ...
    ClassicEditor
        ...
        .then(editor => {
            // Verificar si $blog->extract está definido
            @if(isset($blog) && $blog->extract)
                editor.setData(@json($blog->extract));
            @endif
        })
        .catch(error => {
            console.error('Error en CKEditor extract:', error);
        });
    ...
    ClassicEditor
        ...
        .then(editor => {
            // Verificar si $blog->body está definido
            @if(isset($blog) && $blog->body)
                editor.setData(@json($blog->body));
            @endif
        })
        .catch(error => {
            console.error('Error en CKEditor body:', error);
        });
});
