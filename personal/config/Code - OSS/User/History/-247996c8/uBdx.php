...
class QuotationTable extends DataTableComponent
{
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make("Visualizar")
                ->label(function ($row) {
                    return view('admin.quotes.generate', ['quotaId' => $row]);
                }),
            ...
            Column::make("Estado", "status")
                ->format(function ($value) {
                    $status = is_numeric($value) ? (int) $value : null;
                    return QuotationStatus::tryFrom($status)?->name ?? 'Desconocido';
                })
                ->sortable(),
            ...
            Column::make("Actions")
                ->label(function ($row) {
                    return view('admin.quotes.actions', ['quotaId' => $row]);
                }),
    ...
    public function downloadPdf(Quotation $quotaId)
    {
        if ($quotaId->pdf_path && Storage::exists($quotaId->pdf_path)) {
            return Storage::download($quotaId->pdf_path);
        }
