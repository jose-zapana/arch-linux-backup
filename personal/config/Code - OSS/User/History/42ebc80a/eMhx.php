            ...
            <div class="card dark:bg-gray-800">
                <ul class="space-y-4">
                    ...
                    @forelse (Cart::content() as $item)
                        ...
                        <img src="{{ $item->model->getFirstMediaUrl('images', 'product') ?: asset('img/no-image.png') }}" 
                        alt="{{ $item->name }}" 
        ...
        <div class="lg:col-span-2">
            <div class="card dark:bg-gray-900">
                <div class="flex justify-between font-semibold mb-2">
                    ...
                    <p class="dark:text-gray-300"> S/ {{ $this->subtotal }}</p>
                </div>

                @if (Cart::count())
                <a href="{{ route('shipping.index') }}" class="btn bg-indigo-500 text-white hover:bg-indigo-600 dark:bg-indigo-400 dark:hover:bg-indigo-500 disabled:bg-gray-300 dark:disabled:bg-gray-700 disabled:text-gray-500 dark:disabled:text-gray-400 rounded-lg shadow-lg transition-all block w-full text-center mb-6">
                    Continuar compra
                </a>
            @else
                <a href="{{ route('welcome.index') }}" class="btn bg-indigo-500 text-white hover:bg-indigo-600 dark:bg-indigo-400 dark:hover:bg-indigo-500 disabled:bg-gray-300 dark:disabled:bg-gray-700 disabled:text-gray-500 dark:disabled:text-gray-400 rounded-lg shadow-lg transition-all block w-full text-center mb-6">
                    Ir al inicio
                </a>
            @endif
            </div>
