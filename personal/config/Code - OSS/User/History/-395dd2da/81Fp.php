...
class Product extends Model implements HasMedia
{
    ...
    {
        // Versión pequeña para dispositivos con pantallas pequeñas
        $this->addMediaConversion('mobile_small')
            ->width(360)               // Ancho máximo de 360px
            ->height(360)              // Mantiene la relación de aspecto
            ->format('webp')           // Convertir a WebP
            ->nonQueued();             // Procesar de inmediato
    
        // Versión mediana para pantallas de tamaño promedio
        $this->addMediaConversion('mobile_medium')
            ->width(540)               // Ancho máximo de 540px
            ->height(540)              // Mantiene la relación de aspecto
            ->format('webp')           // Convertir a WebP
            ->nonQueued();             // Procesar de inmediato
    
        // Versión grande para pantallas más grandes (tablets y teléfonos grandes)
        $this->addMediaConversion('mobile_large')
            ->width(720)               // Ancho máximo de 720px
            ->height(720)              // Mantiene la relación de aspecto
            ->format('webp')           // Convertir a WebP
            ->nonQueued();             // Procesar de inmediato
    }
}
