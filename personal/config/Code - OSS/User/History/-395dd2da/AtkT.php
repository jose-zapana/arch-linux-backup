...
class Product extends Model implements HasMedia
{
    ...
    {
        $this
            // Conversión para la imagen principal (ancho máximo de 1080px)
            ->addMediaConversion('preview')
            ->format('webp')          // Convierte a WebP
            ->noQueued()                // Utiliza cola para procesamiento
            ->width(1080)             // Ancho máximo de la imagen
            ->height(1080);           // Mantiene la relación de aspecto si es necesario
        
        // Conversión para miniaturas más pequeñas (600px de ancho)
        $this
            ->addMediaConversion('small')
            ->format('webp')          // Convierte a WebP
            ->queued()                // Utiliza cola para procesamiento
            ->width(600);             // Ancho máximo para miniaturas
    
        // Conversión para vistas previas pequeñas (300px de ancho)
        $this
            ->addMediaConversion('thumb')
            ->format('webp')          // Convierte a WebP
            ->noQueued()                // Utiliza cola para procesamiento
            ->width(300);             // Ancho máximo para thumbnails pequeños
    }
}
