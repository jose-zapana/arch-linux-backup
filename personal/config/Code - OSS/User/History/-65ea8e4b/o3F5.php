...
class BlogController extends Controller
{
    ...
    public function update(PostRequest $request, Post $post)
    {
        ...
        // Actualizar los campos del post (sin tocar el body aún)
        $post->update($data);
        ...
        $existingMedia = $post->getMedia('posts')->pluck('file_name')->toArray();
        ...
        // Eliminar imágenes que ya no están en el contenido (se eliminaron del body o fueron reemplazadas)
        foreach ($existingMedia as $mediaFile) {
            if (!in_array($mediaFile, array_map('basename', $replacedUrls))) {
                // Eliminar el archivo que ya no se usa
                Storage::delete($mediaFile);
                $post->getMedia('posts')->where('file_name', $mediaFile)->each->delete();
            }
        }
    
        // Si hay una nueva imagen destacada, agregarla a Media Library
        if ($request->hasFile('file')) {
