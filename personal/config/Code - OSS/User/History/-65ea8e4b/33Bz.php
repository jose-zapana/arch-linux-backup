...
class BlogController extends Controller
{
    ...
    public function update(PostRequest $request, Post $blog)
    {
        ...
        // Obtener imágenes existentes en el body antes de actualizar
        $existingMedia = $blog->getMedia('posts')->pluck('file_name')->toArray();
        ...
        // Convertir las URLs usadas en nombres de archivos
        $usedMediaFiles = array_map(function ($url) {
            return basename(parse_url($url, PHP_URL_PATH));
        }, $usedImages);
    
        // Eliminar imágenes huérfanas (no usadas en el body)
        foreach ($existingMedia as $fileName) {
            if (!in_array($fileName, $usedMediaFiles)) {
                $mediaItem = $blog->getMedia('posts')->firstWhere('file_name', $fileName);
                if ($mediaItem) {
                    $mediaItem->delete();
                }
            }
        ...
        // Reemplazar las URLs temporales por URLs finales en el contenido del body
        $replacedUrls = [];
        foreach ($usedImages as $imageUrl) {
            ...
            if (file_exists($absolutePath)) {
                ...
                $newUrl = $mediaItem->getUrl();
                $replacedUrls[$imageUrl] = $newUrl;
        ...
        // Si se sube una nueva imagen destacada
        if ($request->hasFile('file')) {
            // Eliminar la imagen destacada anterior
