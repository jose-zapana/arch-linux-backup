...
class BlogController extends Controller
{
    ...
    {   
        ...
        $files = Storage::files('media');  // Obtiene todos los archivos en 'media'
        $images = Image::pluck('image_url')->toArray();  // Obtiene los nombres de las imágenes
        
        // Asegúrate de agregar la ruta completa si solo estás almacenando nombres de archivo
        $filesToDelete = array_diff($files, array_map(function($image) {
            return 'media/' . $image;  // Ajusta esta parte según cómo almacenas las URLs de las imágenes
        }, $images));
    ...
    public function update(PostRequest $request, Post $blog)
    {
        ...
        if ($request->hasFile('file')) {
            // Eliminar la imagen existente en la colección 'posts'
            if ($blog->hasMedia('posts')) {
                $blog->clearMediaCollection('posts');
            }
    
            // Agregar la nueva imagen a la colección 'posts'
            $blog->addMediaFromRequest('file') // Obtiene el archivo desde el request
                ->toMediaCollection('posts');  // Lo guarda en la colección 'posts'
        }
