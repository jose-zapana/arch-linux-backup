     ...
     public function update(PostRequest $request, Post $blog)
     {
         ...
         foreach ($existingMedia as $mediaItem) {
             if (!in_array($mediaItem->file_name, $usedMediaFiles)) {
                 $mediaItem->delete(); // Eliminar solo de Media Library y Storage
             }
         }
     
         // Agregar nuevas imágenes al cuerpo del post
         $replacedUrls = [];
         ...
         // Reemplazar las URLs temporales con URLs finales en el contenido del body
         foreach ($replacedUrls as $oldUrl => $newUrl) {
