...
class BlogController extends Controller
{
    ...
    {   
        $data = $request->all();
        $data['user_id'] = Auth::id(); // Asegúrate de que user_id sea el ID del usuario autenticado
        ...
        foreach ($images as $image) {
            $post->addMediaFromUrl($image)->toMediaCollection('posts');
        }   
        
        if ($request->hasFile('file')) {
            // Guarda la imagen en la colección 'posts' usando Spatie Media Library
            $post->addMediaFromRequest('file') // Obtiene el archivo desde el request
                 ->toMediaCollection('posts'); // Lo guarda en la colección 'posts'
            
            // Refresca el modelo para asegurarte de que la relación esté actualizada
            $post->refresh();
        }
        ...
        // Asegúrate de que la redirección utiliza el slug
